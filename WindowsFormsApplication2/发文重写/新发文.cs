...
namespace TYGDQ
{
	public partial class 新发文 : Form
	{
		...
		private void lbxIni_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			string get = getdata.IniReadValue("发文配置", idget[0].Trim(), "");
			if (get == "")
				return;
			...
			NewSendText.文章地址 = getAll[2];
			NewSendText.标题 = getAll[3];
			NewSendText.标记 = int.Parse(getAll[4]);
			NewSendText.发送字数 = int.Parse(getAll[5]);
			NewSendText.已发字数 = int.Parse(getAll[6]);
			NewSendText.起始段号 = int.Parse(getAll[7]);
			NewSendText.是否乱序 = bool.Parse(getAll[8]);
			NewSendText.是否周期 = bool.Parse(getAll[9]);
			NewSendText.周期长度 = int.Parse(getAll[10]);
			NewSendText.是否独练 = bool.Parse(getAll[11]);

			lblTitle.Text = NewSendText.标题;
			tbxSendStart.Text = NewSendText.标记.ToString();
			tbxSendCount.Text = NewSendText.发送字数.ToString();
			if (NewSendText.是否乱序)
				rbnOutOrder.PerformClick();
			else
				rbninOrder.PerformClick();
			checkBox2.Checked = NewSendText.是否独练;
			checkBox1.Checked = NewSendText.是否周期;
			nudSendTimer.Value = NewSendText.周期长度;
